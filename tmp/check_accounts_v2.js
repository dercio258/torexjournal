...
async function check() {
  ...
  if (user) {
    const accounts = (await client.query("SELECT id, mt5_id, is_connected FROM accounts WHERE user_id = $1", [user.id])).rows;
    ...
    for (const acc of accounts) {
      ...
      console.log(`Account ${acc.id} (MT5 ID: ${acc.mt5_id || 'N/A'}) has ${tradesCount} trades.`);
    }
  } else {
